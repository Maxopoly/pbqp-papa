 ...
 */
template<typename T>
class ConnectedComponentSeparator: public NodeConsistentReduction<T> {
	...
	std::vector<PBQP_Graph<T>*>* reduce() override {
		while (true) {
			...
			while (!todoStack.empty()) {
				...
				for (PBQP_Node<T>* adjacentNode : node->getAdjacentEdges(false)) {
					if (!foundNodes.contains(adjacentNode)) {
						todoStack.insert(adjacentNode);
					}
				}
			}
