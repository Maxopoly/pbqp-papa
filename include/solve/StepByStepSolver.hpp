...
public:
	...
	StepByStepSolver(PBQPGraph<InfinityWrapper<T>>* graph) :
			graph(graph), useRnAlready(false), nodeIndex(0), peoIndex(0), backtrackIndex(0), isBackTracking(false), lastReduction(NONE) {
...
public:
	...
	void applyRN(PBQPNode<InfinityWrapper<T>>* node) {
		...
		DependentSolution<InfinityWrapper<T>>* sol = DegreeNReducer<InfinityWrapper<T>>::reduceRNEarlyDecision(node, this->graph);
		if (!sol) {
